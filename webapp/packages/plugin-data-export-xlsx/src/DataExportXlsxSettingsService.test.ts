...
import { dataExportXlsxManifest } from './manifest';
...
const newSettings = {
  ...deprecatedSettings,
  'plugin.data-xlsx-export.disabled': testValueA,
};

test('New settings override deprecated', async () => {
  const settings = app.injector.getServiceByClass(DataExportXlsxSettingsService);
