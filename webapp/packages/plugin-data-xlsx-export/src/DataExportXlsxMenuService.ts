...
import { ACTION_EXPORT_XLSX, ActionService, DATA_CONTEXT_MENU, menuExtractItems, MenuService } from '@cloudbeaver/core-view';
import {
  DATA_CONTEXT_DV_DDM,
...
import { DataExportXlsxSettingsService } from './DataExportXlsxSettingsService';

const DataExportDialog = importLazyComponent(() => import('./Dialog/DataExportDialog').then(module => module.DataExportDialog));

@injectable()
export class DataExportXlsxMenuService {
  constructor(
    private readonly commonDialogService: CommonDialogService,
    private readonly dataExportSettingsService: DataExportXlsxSettingsService,
  ...
  register(): void {
    this.actionService.addHandler({
      id: 'data-xlsx-export-base-handler',
      isActionApplicable(context, action) {
        ...
        return [ACTION_EXPORT_XLSX].includes(action);
      },
      isDisabled(context) {
    ...
    this.menuService.addCreator({
      ...
      getItems: (context, items) => [...items, ACTION_EXPORT_XLSX],
    });
